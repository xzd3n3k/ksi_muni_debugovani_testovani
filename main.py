...
def decode(morse_text: str) -> str:
    ...
    for i in range(len(morse_array)):
        ...
        current_element = morse_array[i].split('/')

        for elem in current_element:
            plain_text += morse[elem]

        if i != len(morse_array)-1:
            plain_text += ' '
...
print(decode(encode(
    "Do translation dict jsou pridany vsechny specialni znaky a cisla, ktera budete potrebovat. To stejne nerikam o malych pismenech.")))
print(encode("ab c"))  # .-/-...//-.-.///
